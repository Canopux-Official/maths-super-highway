...
import {
    Dialog, DialogTitle, DialogContent, DialogActions, Divider,
    Button, TextField, MenuItem, Box, Typography, Stack,
    CircularProgress, IconButton, Backdrop
} from '@mui/material';
import DeleteIcon from '@mui/icons-material/Delete';
import CloudUploadIcon from '@mui/icons-material/CloudUpload';
import type { CourseFormState } from './types';
...
const CourseFormDialog: React.FC<CourseFormDialogProps> = ({
    ...
    return (
        <Dialog
            open={open}
            onClose={isUploading ? undefined : onClose} // prevent closing during upload
            fullWidth
            maxWidth={form.itemType === 'page' ? 'md' : 'sm'}
            slotProps={{ paper: { sx: { borderRadius: 3, mx: { xs: 2, sm: 'auto' }, overflow: 'visible' } } }}
        >
            {/* ── Upload Overlay Backdrop (scoped inside Dialog) ── */}
            <Backdrop
                open={isUploading}
                sx={{
                    position: 'absolute',
            ...
            >
                ...
                <Typography
                    variant="h6"
                    sx={{ fontWeight: 700, textAlign: 'center', letterSpacing: 0.3 }}
                >
                    Uploading… Please wait
                </Typography>
                <Typography
                    variant="body2"
                    sx={{ opacity: 0.75, textAlign: 'center', maxWidth: 280 }}
                >
            ...
            <DialogActions sx={{ p: { xs: 2, sm: 3 }, gap: 1 }}>
                ...
                {/* Hide Save/Create button entirely while uploading */}
                {!isUploading && (
                    <Button
                        variant="contained"
                        onClick={onSubmit}
                        disabled={!form.title.trim() || (form.itemType === 'page' && !form.content)}
                    >
                        {editingId ? 'Save Changes' : 'Create'}
                    </Button>
                )}
