...
const FontSize = Extension.create({
  ...
  addCommands() {
    return {
      // ✅ FIX: added extendMarkRange('textStyle') before setMark so the mark
      //    is properly merged even when bold (or other marks) are active.
    ...
    <span>
      <IconButton
        ...
        sx={{
          ...
          '&:hover': {
            bgcolor: active ? 'rgba(29,78,216,0.12)' : 'rgba(100,116,139,0.08)',
          ...
          >
            {!c.value && (
              <Box sx={{
                ...
                bgcolor: '#DC2626', transform: 'rotate(-45deg)', borderRadius: 1,
              }} />
            )}
          </Box>
        </Tooltip>
...
const LinkDialog = ({
  ...
  return (
    ...
    >
      <Box sx={{
        fontSize: '0.7rem', fontWeight: 700, color: '#94A3B8',
        mb: 1.25, textTransform: 'uppercase', letterSpacing: '0.06em',
      }}>
        Insert Link
      </Box>
      <TextField
      ...
      <Box sx={{ display: 'flex', gap: 1 }}>
        <Button
          ...
          sx={{ borderRadius: '7px', textTransform: 'none', fontSize: '0.8rem', bgcolor: '#1D4ED8', flex: 1 }}
        >
          Apply
        </Button>
        {currentUrl && (
          <Button
            ...
            onClick={() => { onUnset(); onClose(); }}
            sx={{ borderRadius: '7px', textTransform: 'none', fontSize: '0.8rem' }}
          >
            Remove
          </Button>
        )}
...
const Editor = ({ value, onChange }: EditorProps) => {
  ...
  const editor = useEditor({
    extensions: [
      // ✅ FIX: Explicitly disable StarterKit's built-in textStyle (if any) to
      //    prevent double-registration conflicts with our standalone TextStyle.
      StarterKit.configure({
        // StarterKit doesn't bundle textStyle by default, but being explicit
        // ensures no hidden conflict with our separately added TextStyle.
      }),
      Underline,
      // ✅ TextStyle MUST come before Color and FontFamily so the mark is
      //    registered before those extensions try to extend it.
      TextStyle,
      Color,
      FontFamily,
      FontSize,
      ...
      <Box sx={{
        ...
        {(['Normal', 'H1', 'H2', 'H3'] as const).map(level => {
          const isActive = level !== 'Normal'
            && editor.isActive('heading', { level: Number(level.slice(1)) as 1 | 2 | 3 });
          return (
            <Tooltip key={level} title={level === 'Normal' ? 'Normal text' : `Heading ${level.slice(1)}`} placement="top" arrow>
              <Box
                component="button"
                ...
                sx={{
                  ...
                  px: 0.875, height: 28, borderRadius: '6px',
        ...
        <Tooltip title="Font family" placement="top" arrow>
          <Select
            ...
            displayEmpty
            sx={{
              height: 28, fontSize: '11px', fontWeight: 600,
          ...
          >
            {FONT_FAMILIES.map(f => (
              <MenuItem key={f.value} value={f.value} sx={{ fontSize: '12px', fontFamily: f.value || 'inherit' }}>
                {f.label}
              </MenuItem>
            ))}
          </Select>
        ...
        <Tooltip title="Font size" placement="top" arrow>
          ...
          >
            ...
            {FONT_SIZES.map(s => (
              <MenuItem key={s} value={s} sx={{ fontSize: '12px' }}>{s}px</MenuItem>
            ))}
          </Select>
        </Tooltip>
        ...
        {/* Text Formatting */}
        <ToolbarButton label="Bold (Ctrl+B)" active={editor.isActive('bold')} onClick={() => editor.chain().focus().toggleBold().run()}>
          <FormatBold sx={{ fontSize: 15 }} />
        </ToolbarButton>
        <ToolbarButton label="Italic (Ctrl+I)" active={editor.isActive('italic')} onClick={() => editor.chain().focus().toggleItalic().run()}>
          <FormatItalic sx={{ fontSize: 15 }} />
        </ToolbarButton>
        <ToolbarButton label="Underline (Ctrl+U)" active={editor.isActive('underline')} onClick={() => editor.chain().focus().toggleUnderline().run()}>
          <FormatUnderlined sx={{ fontSize: 15 }} />
        </ToolbarButton>
        <ToolbarButton label="Strikethrough" active={editor.isActive('strike')} onClick={() => editor.chain().focus().toggleStrike().run()}>
          <StrikethroughS sx={{ fontSize: 15 }} />
        </ToolbarButton>
        ...
        <Tooltip title="Text color" placement="top" arrow>
          <Box
            component="button"
            ref={textColorBtnRef}
            onClick={() => setTextColorAnchor(textColorBtnRef.current)}
            sx={{
              width: 28, height: 28, borderRadius: '6px',
              border: '1px solid transparent', cursor: 'pointer',
              bgcolor: 'transparent',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              flexDirection: 'column', gap: '2px', p: 0,
          ...
          >
            ...
            <Box sx={{
              width: 16, height: 3, borderRadius: '1px',
              bgcolor: activeTextColor,
        ...
        <ColorPicker
          ...
          title="Text Color"
          onSelect={val => {
            // ✅ FIX: extendMarkRange('textStyle') ensures the color mark is
            //    applied across the full selection even when bold is active.
            //    Without this, setColor silently fails on bold text because
            //    the textStyle mark range doesn't cover the bold mark range.
            if (!val) {
              editor.chain().focus().extendMarkRange('textStyle').unsetColor().run();
            } else {
              editor.chain().focus().extendMarkRange('textStyle').setColor(val).run();
            }
          }}
        />
        ...
        <Tooltip title="Highlight color" placement="top" arrow>
          <Box
            component="button"
            ref={highlightBtnRef}
            onClick={() => setHighlightAnchor(highlightBtnRef.current)}
            sx={{
              width: 28, height: 28, borderRadius: '6px',
              border: editor.isActive('highlight') ? '1px solid rgba(29,78,216,0.2)' : '1px solid transparent',
              cursor: 'pointer',
              bgcolor: editor.isActive('highlight') ? 'rgba(29,78,216,0.08)' : 'transparent',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              flexDirection: 'column', gap: '2px', p: 0,
              '&:hover': { bgcolor: 'rgba(100,116,139,0.08)' },
              transition: 'all 0.12s',
        ...
        <Tooltip title={editor.isActive('link') ? 'Edit link' : 'Add link'} placement="top" arrow>
          <span ref={linkBtnRef}>
            <IconButton
              size="small"
              onClick={() => setLinkAnchor(linkBtnRef.current)}
              sx={{
                borderRadius: '6px', width: 28, height: 28,
                color: editor.isActive('link') ? '#1D4ED8' : '#64748B',
                bgcolor: editor.isActive('link') ? 'rgba(29,78,216,0.08)' : 'transparent',
                border: editor.isActive('link') ? '1px solid rgba(29,78,216,0.2)' : '1px solid transparent',
                '&:hover': { bgcolor: 'rgba(100,116,139,0.08)' },
                transition: 'all 0.12s',
              }}
            >
        ...
        {editor.isActive('link') && (
          <ToolbarButton label="Remove link" onClick={unsetLink}>
        ...
        <ToolbarButton label="Numbered list" active={editor.isActive('orderedList')} onClick={() => editor.chain().focus().toggleOrderedList().run()}>
          <FormatListNumbered sx={{ fontSize: 15 }} />
        </ToolbarButton>
        <ToolbarButton label="Blockquote" active={editor.isActive('blockquote')} onClick={() => editor.chain().focus().toggleBlockquote().run()}>
          <FormatQuote sx={{ fontSize: 15 }} />
        </ToolbarButton>
        <ToolbarButton label="Code block" active={editor.isActive('codeBlock')} onClick={() => editor.chain().focus().toggleCodeBlock().run()}>
          <Code sx={{ fontSize: 15 }} />
        </ToolbarButton>
        <ToolbarButton label="Inline code" active={editor.isActive('code')} onClick={() => editor.chain().focus().toggleCode().run()}>
          <CodeOff sx={{ fontSize: 15 }} />
        </ToolbarButton>
        <ToolbarButton label="Horizontal rule" onClick={() => editor.chain().focus().setHorizontalRule().run()}>
      ...
      <Box sx={{
        ...
        '& .tiptap': {
          ...
          '& blockquote': {
            ...
            paddingLeft: '1rem', paddingRight: '0.75rem',
            paddingTop: '0.6rem', paddingBottom: '0.6rem',
            marginLeft: 0, marginTop: '1em', marginBottom: '1em',
          ...
          '& strong': { fontWeight: 700, color: 'inherit' }, // ✅ FIX: changed from '#0F172A' to 'inherit'
          // so that color applied on bold text is not overridden by a hardcoded strong color.
          '& em': { fontStyle: 'italic' },
        },
      }}>
        <EditorContent editor={editor} />
